...
hex ::
  Parser Char
hex = chr . toHexInt <$> replicateA 4 (satisfy isHexDigit)
  where toHexInt s = case readHex s of
          Empty -> 0
          Full i -> i
...
sepby1 ::
  ...
  -> Parser s
  -> Parser (List a)
sepby1 a s = a >>= (\a'-> (list $ s >>> a) >>= (\as -> return (a':.as)))
...
sepby ::
  ...
  -> Parser s
  -> Parser (List a)
sepby a s = sepby1 a s ||| return Nil
...
satisfyAll ::
  List (Char -> Bool)
  -> Parser Char
satisfyAll fs = satisfy (\s-> and $ map ($ s) fs)
...
betweenSepbyComma ::
  ...
  -> Parser a
  -> Parser (List a)
betweenSepbyComma s e a = betweenCharTok s e (sepby a $ charTok ',')
