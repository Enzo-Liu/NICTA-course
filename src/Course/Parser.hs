...
bindParser ::
  ...
  -> Parser a
  -> Parser b
bindParser f (P pa) = P $ \s-> case pa s of
...
-- Result >< ""
list' :: (Input -> ParseResult a) -> List a  -> Input -> ParseResult (List a)
list' f ls s = case f s of
  ErrorResult _ -> Result s (reverse ls)
  Result s' r  -> list' f (r:.ls) s'

list ::
  Parser a
  -> Parser (List a)
list (P a) = P (list' a Nil)
...
list1 ::
  Parser a
  -> Parser (List a)
list1 a = bindParser atLeast1 (list a)
  where atLeast1 Nil = P $ \_->ErrorResult UnexpectedEof
        atLeast1 l@(_:._) = valueParser l
...
satisfy ::
  ...
  -> Parser Char
satisfy p = P p'
  where p' Nil = ErrorResult UnexpectedEof
        p' (x:.xs) = if p x then Result xs x else ErrorResult (UnexpectedChar x)
...
natural ::
  Parser Int
natural = flbindParser (list1 digit) $ \li -> case read li of
  Empty -> failed
...
sequenceParser ::
  List (Parser a)
  -> Parser (List a)
sequenceParser lp = foldRight
  (\p pl ->
     bindParser (\x -> flbindParser pl (\xs -> valueParser (x:.xs))) p)
  (valueParser Nil) lp
...
firstNameParser ::
  Parser Chars
firstNameParser = flbindParser upper $ \ a ->
  flbindParser (list lower) $ \as -> valueParser (a:.as)
...
surnameParser = flbindParser upper $ \a ->
  flbindParser (thisMany 5 lower) $ \as ->
  flbindParser (list lower) $ \ass -> valueParser (a:.as ++ ass)
...
phoneParser ::
  Parser Chars
phoneParser = flbindParser digit $ \d->
  flbindParser phoneBodyParser $ \ds ->
  flbindParser (is '#') $ \_ -> valueParser (d:.ds)
...
personParser = flbindParser ageParser (\age' ->
  spaces1 >>> flbindParser firstNameParser (\firstName' ->
  spaces1 >>> flbindParser surnameParser (\surname' ->
  spaces1 >>> flbindParser smokerParser (\smoker' ->
  spaces1 >>> flbindParser phoneParser
...
instance Applicative Parser where
  ...
  (<*>) ::
    ...
    -> Parser a
    -> Parser b
  (<*>) f a = bindParser (\f'-> bindParser (valueParser . f') a)  f
