...
findRight p (ListZipper l x r) = case break p r of
    (_, Nil) ->
      IsNotZ
    (l', x':.r') ->
      IsZ $ ListZipper (reverse l' ++ x :. l) x' r'
...
moveLeftLoop ::
  ListZipper a
  -> ListZipper a
moveLeftLoop (ListZipper Nil a y) = let (x:.xs) = reverse (a:.y) in ListZipper xs x Nil
moveLeftLoop (ListZipper (x:.xs) a y) = ListZipper xs x (a:.y)
...
moveRightLoop ::
  ListZipper a
  -> ListZipper a
moveRightLoop (ListZipper x a Nil) = let (y:.ys) = reverse (a:.x) in ListZipper Nil y ys
moveRightLoop (ListZipper x a (y:.ys)) = ListZipper (a:.x) y ys
...
moveLeft (ListZipper (x:.xs) a y) = IsZ $ ListZipper xs x (a:.y)
...
moveRight (ListZipper x a (y:.ys)) = IsZ $ ListZipper (a:.x) y ys
...
swapLeft (ListZipper (x:.xs) a y) = IsZ $ ListZipper (a:.xs) x y
...
swapRight (ListZipper x a (y:.ys)) = IsZ $ ListZipper x y (a:.ys)
...
moveLeftN ::
  ...
  -> ListZipper a
  -> MaybeListZipper a
moveLeftN n l | n == 0 = IsZ l
moveLeftN n l | n < 0 = moveRightN (negate n) l
moveLeftN n l = moveLeftN (n-1) -<< moveLeft l
...
moveRightN ::
  ...
  -> ListZipper a
  -> MaybeListZipper a
moveRightN n l | n == 0 = IsZ l
moveRightN n l | n < 0 = moveLeftN (negate n) l
moveRightN n l = moveRightN (n-1) -<< moveRight l
...
moveLeftN' n l = moveLeftN'' n l 0
  where moveLeftN'' 0 l' _  = Right l'
        moveLeftN'' n' _ _| n<0  = moveRightN' (negate n') l
        moveLeftN'' n' l' r  = case moveLeft l' of
          IsZ l'' -> moveLeftN'' (n'-1) l'' (r+1)
          IsNotZ  -> Left r
...
moveRightN' ::
  ...
  -> ListZipper a
  -> Either Int (ListZipper a)
moveRightN' n l = moveRightN'' n l 0
  where moveRightN'' 0 l' _  = Right l'
        moveRightN'' n' _ _| n<0  = moveLeftN' (negate n') l
        moveRightN'' n' l' r  = case moveRight l' of
          IsZ l'' -> moveRightN'' (n'-1) l'' (r+1)
          IsNotZ  -> Left r
...
nth n lz@(ListZipper l _ _)
  | n < 0 = IsNotZ
  | otherwise = moveLeftN (length l - n) lz
...
deletePullLeft (ListZipper (x:.xs) _ y) = IsZ $ ListZipper xs x y
...
deletePullRight (ListZipper x _ (y:.ys)) = IsZ $ ListZipper x y ys
...
insertPushLeft ::
  ...
  -> ListZipper a
  -> ListZipper a
insertPushLeft i (ListZipper l a r)= ListZipper (a:.l) i r
...
insertPushRight ::
  ...
  -> ListZipper a
  -> ListZipper a
insertPushRight i (ListZipper l a r) = ListZipper l i (a:.r)
...
-- /Tip:/ Use `zipWith`
  (ListZipper fl f fr) <*> (ListZipper al a ar ) = ListZipper (zipWith id fl al) (f a) (zipWith id fr ar)
...
instance Traversable ListZipper where
  traverse f (ListZipper l a r) = (ListZipper . reverse) <$> traverse f (reverse l) <*> f a <*> traverse f r
...
instance Traversable MaybeListZipper where
  ...
  traverse f (IsZ l) = IsZ <$> traverse f l
