...
instance Functor f => Functor (StateT s f) where
  (<$>) ::
    ...
    -> StateT s f a
    -> StateT s f b
  f <$> StateT st = StateT $ \s -> (\(a,s')->(f a,s')) <$> st s
...
instance Monad f => Monad (StateT s f) where
  (=<<) ::
    ...
    -> StateT s f a
    -> StateT s f b
  f =<< (StateT a) = StateT $ (=<<) (\(a',s')-> runStateT (f a') s') . a
...
distinct' ::
  ...
  List a
  -> List a
distinct' l = eval' (filtering notContains l) S.empty
  where notContains a = state' (S.notMember a &&& S.insert a)
...
distinctF ::
  ...
  List a
  -> Optional (List a)
distinctF l = evalT (filtering notContains l) S.empty
  where notContains a | a > 100 = StateT $ \_ -> Empty
        notContains a = StateT $ \s -> Full  ( S.notMember a s, S.insert a s)
...
instance Monad f => Monad (OptionalT f) where
  f =<< (OptionalT a) = OptionalT $ a >>= f'
    where f' Empty = return Empty
